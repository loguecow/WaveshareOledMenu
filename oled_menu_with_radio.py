...
def start_fm_transmitter():
    ...
    frequency = 100.6
    sound_file = "./fm_transmitter/acoustic_guitar_duet.wav"
    
    while True:
        with canvas(device) as draw:
            draw.text((0, 0), "FM Transmitter", font=font_bold, fill="white")
            draw.text((0, 15), f"Freq: {frequency:.1f} MHz", font=font, fill="white")
            draw.text((0, 25), f"File: {sound_file}", font=font, fill="white")
            draw.text((0, 45), "Up/Down: Freq", font=font, fill="white")
            draw.text((0, 55), "Press: Start/Stop", font=font, fill="white")
        
        time.sleep(0.1)
        
        if GPIO.input(KEY_UP_PIN) == GPIO.LOW:
            frequency = min(108.0, frequency + 0.1)
            last_activity_time = time.time()
            time.sleep(debounce_time)
        elif GPIO.input(KEY_DOWN_PIN) == GPIO.LOW:
            frequency = max(87.5, frequency - 0.1)
            last_activity_time = time.time()
            time.sleep(debounce_time)
        elif GPIO.input(KEY_PRESS_PIN) == GPIO.LOW:
            if fm_process is None:
                ...
                with canvas(device) as draw:
                    draw.text((0, 35), "Transmitting...", font=font, fill="white")
            else:
                # Stop FM transmission
                fm_process.terminate()
                fm_process = None
                with canvas(device) as draw:
                    draw.text((0, 35), "Stopped", font=font, fill="white")
            last_activity_time = time.time()
            time.sleep(debounce_time)
        elif GPIO.input(KEY_LEFT_PIN) == GPIO.LOW:
            if fm_process:
